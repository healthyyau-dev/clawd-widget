...
// ---------- notification sounds ----------
// One chime when a session ENTERS 'done', another when it enters 'question'. Preloaded so the first
// play has no fetch delay. Paths are relative to renderer/index.html and work both in dev and inside
// the packaged asar. Autoplay is allowed via the window's autoplayPolicy (set in main.js).
const SOUNDS = {
  done: new Audio('../assets/sounds/complete.mp3'),
  question: new Audio('../assets/sounds/pending-question.mp3')
}
Object.values(SOUNDS).forEach((a) => { a.preload = 'auto'; a.volume = 0.6 })
// Chime on completion and on each pending question. Keyed by a per-sound SIGNATURE (not just the
// state name) so EVERY distinct question chimes -- including back-to-back questions with new content,
// not only the first -- while a steady state or an identical re-push never repeats. A non-sound state
// (working/default/...) resets the signature so the next done/question chimes again.
let lastSoundSig = null
function playStateSound (state) {
  const st = state && state.state
  if (st !== 'done' && st !== 'question') { lastSoundSig = null; return }
  const sig = st === 'question'
    ? 'q|' + (state.question || '') + '|' + (state.questions ? state.questions.length : 0)
    : 'done'
  if (sig === lastSoundSig) return
  lastSoundSig = sig                          // record even when muted, so toggling on mid-state doesn't retro-chime
  if (prefs && prefs.sfx === false) return    // Sound effects turned off in the menu
  const a = SOUNDS[st]
  if (!a) return
...
// Live pref updates (e.g. the menu's Sound effects toggle) -- merge so playback reflects it at once.
window.clawd.onPrefs((p) => { if (p && typeof p === 'object') prefs = Object.assign({}, prefs, p) })
...
window.clawd.onState((state) => {
  ...
  current = state
  // Chime on completion / each distinct pending question (see playStateSound).
